...
#### [ ] P2. Manual import runs sequential N+1 Supabase round-trips per leg
...
- **Acceptance:** End-to-end POST time for a 200-leg manual import against the QA user dataset, measured server-side from request entry to response, drops below 3s p95. Verified with the Excel template in `docs/qa-test-user.md` filled to 200 legs and run 5× consecutively (timings logged from `route.ts` entry / exit). No more than O(distinct-trades) Supabase round-trips total — counted by adding a request-scoped counter in `createAdminClient` for the duration of the test.

#### [ ] P3. Research dashboard rebroadcasts the entire filtered-trade array to chat context on every filter keystroke
...
#### [ ] P4. `processExecutions` is fully sequential across executions
...
#### [ ] P5. IBKR cron iterates over `BrokerConnection` rows sequentially
...
#### [ ] P6. `ChatContext` value is a fresh object literal on every provider render
...
- **Issue:** Every cron run writes an `xml.slice(0, 10000)` audit blob into `BrokerEvent` per connection. At 2 cron runs × 365 days × N users this becomes a meaningful row of storage growth — not a query-latency issue today (the audit table isn't selected from in hot paths), but it will eventually become an operational concern. Pure hygiene.
...
- **Chat "full" mode — what's the right product shape?** P1 forces a choice: do we (a) cap to the last N trades regardless of query, (b) pre-aggregate per-ticker / per-setup rollups server-side and only send raw rows when the question demands them, or (c) move to a tool-use / function-calling shape where Gemini requests specific slices? Each has different latency, cost, and answer-quality trade-offs. Needs a call before P1 can land cleanly.
- **What's the actual user-count target for the next 12 months?** P5 (cron parallelism) is worth doing today even at single-user scale because the change is small, but the priority of P4 (FIFO parallelism) depends on whether monthly active users go from 1 → 10 or 1 → 200 in the planning window.
- **Is the `Order.rawPayload` JSONB column actually needed for any downstream consumer?** P8 prescribes either shrinking or replacing it, but if there's an in-flight or planned consumer (audit, reconciliation, future re-parse with an updated Flex parser) that needs the full payload, the trade-off shifts. Needs a product / ops decision.
- **Is there a real-world latency SLO for the manual-import POST?** P2 picks "p95 < 3s for a 200-leg import" as a target based on observed user complaints, but the actual budget for that operation (5s? 10s? "as long as it eventually completes"?) is a product call.

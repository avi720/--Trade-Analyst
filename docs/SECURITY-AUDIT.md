...
#### [ ] X5. `/api/trades/import` accepts unbounded file uploads
...
#### [ ] X7. `legs` arrays in manual-trade routes are unvalidated and unbounded
...
#### [ ] X9. Admin (service-role) client used where RLS would suffice
...
#### [ ] X10. Manual-trade date/time inputs are concatenated into `Date()` without format validation
...
#### [ ] X11. `CRON_SECRET` is compared with non-constant-time `!==`
...
- **`FLEX_TOKEN_ENCRYPTION_KEY` storage location.** Is the encryption key stored separately from `SUPABASE_SERVICE_ROLE_KEY` (e.g., different Vercel environment scopes, different access lists)? If both leak together the encryption layer adds zero defence — worth confirming or restructuring.
...
- **Audit-log retention.** Is there an intent to keep an immutable audit log of sensitive account changes (email change, password change, account deletion, IBKR token rotation)? None exists today. A small `AuditEvent` table would be the natural home and would complement X1–X3's re-auth requirement.

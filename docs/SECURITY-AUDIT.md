...
#### [x] X17. No audit log for sensitive account changes
- **Where:** [`lib/audit/log.ts`](../lib/audit/log.ts) (new) · [`app/api/auth/change-password/route.ts`](../app/api/auth/change-password/route.ts) · [`app/api/auth/change-email/route.ts`](../app/api/auth/change-email/route.ts) · [`app/api/auth/delete-account/route.ts`](../app/api/auth/delete-account/route.ts) · [`app/api/chat/route.ts`](../app/api/chat/route.ts)
- **Issue:** After Phase 1 (re-auth) and X4 (rate-limit) landed, the app had three lines of defence on sensitive operations but no record of what actually happened. If an attacker bypassed both (stolen device, malicious browser extension, MITM), there was nothing to forensically reconstruct — no record of "the password was changed on Tuesday at 14:03 from IP 1.2.3.x". The owner reconsidered the original Open Question on audit-log retention and approved a minimal `AuditEvent` table, primarily as a defence-in-depth complement to X1–X4 and as a hook for future email notifications ("we noticed your password changed; if not you, click here"). Out of scope for this finding: a `/profile` UI to view the events (defer until a second user exists), retention/cleanup cron (defer until DB grows), and email notifications (requires Resend/SendGrid — separate work).
- **Acceptance:** Migration `add_audit_event_table` creates an RLS-locked `AuditEvent` table (users can `SELECT` own rows via `auth.uid() = userId`; only service-role inserts). `lib/audit/log.ts:logAuditEvent` is fire-and-forget (never throws past Supabase's error path, never blocks the calling handler) and truncates client IPs to /24 (IPv4) or /48 (IPv6) for privacy. Five event types are logged across four routes: `password_changed`, `email_changed`, `account_deleted`, `reauth_failed`, `rate_limit_hit`. Verified by triggering a wrong-password change-password attempt and observing a `reauth_failed` row in `AuditEvent` for the caller's `userId` with `metadata.action = "change-password"` and a truncated `ipAddress`. **Known limitation:** `account_deleted` rows cascade away with the auth user on actual deletion (FK ON DELETE CASCADE) — the row exists only for the brief window between log and delete. Documented; not a defect for a single-user app.

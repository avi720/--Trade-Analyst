...
#### [ ] T17. Next 14.x carries seven unfixed production CVEs that only resolve in Next 15+
...
- **Issue:** `CLAUDE.md` documents the price sync as "currently disabled" and notes the GitHub Actions workflow was never added. The TypeScript still compiles, the routes still respond, and the cron endpoint will accept a `CRON_SECRET`-authenticated call and burn Massive API credits. The half-disabled state means a future Claude session can re-enable the sync by accident (e.g., by adding the missing workflow file based on `CLAUDE.md` history). Gated on Open Question 1.
...
#### [ ] T14. `/dashboard` route and `open-positions-dashboard.tsx` ship behind a redirect
...
Items that surfaced during the audit but need a product or owner decision before they can become actionable findings. T13 and T14 are gated on the first two.

- **1. Is the Massive (formerly Polygon) price sync coming back, and when?** The code (≈ 272 LOC across `lib/massive/`, three API routes, and UI hooks) is documented in `CLAUDE.md` as parked. If the answer is "within the next quarter", leave the routes mounted and add the GitHub Actions workflow. If the answer is "no" or "no plan", delete the routes and the UI hooks. The half-state is the worst of both.
- **2. Is the live `/dashboard` (open positions) view coming back?** Today the route is hidden behind redirects from `app/page.tsx`, `middleware.ts`, login, and auth callback, but `app/(dashboard)/dashboard/page.tsx` and `components/open-positions-dashboard.tsx` (369 LOC) still ship. Same question as 1 — keep or delete, no middle ground.
- **3. Should the IBKR cron stay on GitHub Actions or move to Vercel Cron?** Affects how T9 (cron observability) is scoped — Vercel Cron has built-in delivery + retry semantics, GitHub Actions needs a separate notifier. Either is fine; the question is whether to lean on Vercel's platform or keep cron infra in the repo.
- **4. Is `xlsx` an acceptable swap target?** T1 requires removing `xlsx`. The user-visible surface is "upload an Excel file" + "download the Excel template", which is plausible against `exceljs`, `read-excel-file`, or even a server-rendered CSV path. Owner input: any constraint on what the import accepts beyond the current `.xlsx` happy path (e.g., must we keep parsing `.xls`, `.csv`, mixed encodings)?

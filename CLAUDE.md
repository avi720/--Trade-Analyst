...
- **AI-import worker** (`.github/workflows/ai-import-worker.yml`): drains up to 5 queued `ExcelImportJob`s per run. Triggered on-demand via `repository_dispatch` from the upload route (when `AI_IMPORT_DISPATCH_*` are set) and by a `*/5 * * * *` safety schedule. `concurrency` guard prevents overlapping drains. Claims are atomic (`claim_excel_import_job()` RPC, `FOR UPDATE SKIP LOCKED`). **AI-import watchdog** (`*/15`): fails jobs stuck in an in-flight state >15 min (`errorMessage='timeout_watchdog'`). **AI-import cleanup** (daily `0 3`): removes uploaded xlsx files for terminal jobs older than 7 days (job row kept as audit). Watchdog + cleanup are curl-to-Vercel like `ibkr-sync.yml`.
